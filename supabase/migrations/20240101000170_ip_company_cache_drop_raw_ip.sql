...
--
-- Step 2 of 2 (see migration 0169). Run this ONLY after scripts/backfill-ip-hash.ts
-- has populated ip_hash for every retained row on the same database. It:
--   1. deletes any rows still missing ip_hash (unbackfilled or pre-key stale
--      rows — safe to drop, they just re-query on next lookup),
