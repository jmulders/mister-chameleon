...
| 14 | Filteren op scores diagnosticeert niets; je hebt de **verdeling van scores over echte sessies** nodig + per regel hoe vaak hij vuurde | **Beide gebouwd + live op dev:** scoreverdeling-paneel (per as, met flag als een as niet discrimineert) + regel-vuringen. Na terechte kritiek omgebouwd naar een **dagteller per regel**, en vervolgens (2e ronde kritiek: hot-row-contentie + fail-open) naar **in-memory bufferen met een flush per minuut** — één schrijf per regel per minuut i.p.v. per vuring, geen rijvergrendeling op het hete pad, en de recorder is synchroon + fail-open (raakt alleen een Map, houdt de beslissing nooit op). Batched increment (`increment_rule_fire(…, p_count, p_day)`) end-to-end geverifieerd op dev. Prod bij release. | Opgelost (live op dev) |
...
- **Migratietracking synchroniseren (#14).** Dev: `rule_fire_events` live (via connector), pad geverifieerd; 028/038 self-healing gemaakt en bevestigd. Prod-check gedaan: `rule_fire_events` bestaat er al, maar 028/038/162 staan **niet** in prod's `_migrations` — een prod-`db:migrate` zou dus dezelfde fouten geven zonder de fix. Prod heeft geen dubbele rijen (veilig). De directe schema-schrijf naar prod via de connector wordt door de automatische veiligheidsklasse geblokkeerd (ook na akkoord in de chat — dat vereist een permission-regel in de Claude Code-settings). Resteert: op **dev** één `npm run db:migrate` om 028/038/162 weg te schrijven, en op **prod** hetzelfde met de prod-URL actief — de gefixte migratiebestanden passen de self-heal + seeds zelf idempotent toe (dev-patroon geverifieerd, prod heeft geen dubbele rijen). Óf: permission-regel toevoegen, dan past de connector het toe zoals op dev.
