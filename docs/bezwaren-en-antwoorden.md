...
- **Migratie draaien voor regel-vuringen (#14).** De code staat; `npm run db:migrate` activeert de `rule_fire_events`-tabel. Tijdens het draaien bleken twee **bestaande** migraties (028, 038) kapot door schema-drift (42P10 ontbrekende unique-index / 42703 ontbrekende kolom `name`) — nu self-healing en idempotent gemaakt. Resteert: opnieuw draaien (de 429's zijn tijdelijke rate-limits; de run is incrementeel, dus herhalen loopt vanzelf door) of de tabel los toepassen via de Supabase-connector. Tot dan tonen de tellingen nul.
