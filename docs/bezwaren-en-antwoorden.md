...
- **Migratietracking synchroniseren (#14).** `rule_fire_events` is op dev live (via de connector) en het pad is geverifieerd; twee bestaande migraties (028, 038) waren kapot door schema-drift en zijn nu self-healing + idempotent gemaakt en op dev bevestigd. Resteert alleen nog: één `npm run db:migrate` om 028/038/162 in de app-eigen `_migrations`-tracking weg te schrijven (draait nu foutloos), en dezelfde migraties op **prod** bij release.
