...
| D3 | **Snippet render-modes + design tokens** — per-variant toggle tussen content-swap (default) en gestyled block met huisstijl via tokens. **Snippet + contract + datamodel gebouwd (19 juli):** snippet doet block-mode (`data-mc-block`, HTML-injectie, tokens als scoped CSS-vars); response-contract laat blocks toe (`SlotValue = string \| BlockSlot`); `renderMode`/`blockHtml` staan op hero, cta, proof én feature, en de decide-route emit voor elk een block-slot (`slots.hero`/`.cta`/`.proof`/`.feature`) met de tokens uit `tokenRef` als CSS-vars — alles met tests. **Resteert:** (1) conversion/notification (die missen nog `tokenRef`, dus eerst die laag); (2) blocks *authoren* — `blockHtml`/`renderMode` invullen in de CMS-repo's (Statamic e.a.) + admin-toggle; (3) AI-generatie een block laten leveren (haakt op D1). **Ontwerp: `docs/design/snippet-render-modes.md`.** | Rijkere personalisatie op externe sites, on-brand | — | 🔨 Platform-kant klaar (hero/cta/proof/feature), authoring volgt |
