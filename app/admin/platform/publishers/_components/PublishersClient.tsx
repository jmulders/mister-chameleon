...
import {
  setPublisherRevshareAction,
  setPublisherAccountAction,
  recordPublisherPayoutAction,
            ...
            <tbody>
              {initial.publishers.map((p) => (
                <PublisherRowView key={p.domain} row={p} defaultPct={initial.defaultRevsharePct}
                  canEdit={initial.isSuperAdmin} pending={pending} run={run}
...
function PublisherRowView({ row, defaultPct, canEdit, pending, run, open, onToggle }:
  { row: PublisherRow; defaultPct: number; canEdit: boolean; pending: boolean; run: RunFn; open: boolean; onToggle: () => void }) {
      ...
      {open && (
        <tr>
          <td colSpan={7} className="bg-neutral-50/60 p-3">
            {!canEdit ? (
              <p className="text-xs text-neutral-500">Only platform admins can edit publisher accounts and payouts.</p>
            ) : (
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button className={btn} disabled={pending}
                      onClick={() => run(() => setPublisherRevshareAction(row.domain, rev.trim() === "" ? null : Math.min(100, Math.max(0, Number(rev)))))}>
                      Save
                    </button>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <button className={btn} disabled={pending}
                      onClick={() => run(() => setPublisherAccountAction(row.domain, { name, contactEmail: email, vatNumber: vat, cocNumber: coc, payoutNotes: notes }))}>
                      Save details
                    </button>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <button className={btn + " mt-1.5"} disabled={pending}
                    onClick={() => run(() => recordPublisherPayoutAction(row.domain, Math.round(Number(payAmt) * 100), payNote))}>
                    Record payout
                  </button>
