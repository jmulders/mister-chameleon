  ...
  return (
    <div className="p-8 max-w-4xl space-y-6">
      <AdminPageHeader
        ...
        description="Email addresses that must no longer be contacted (unsubscribes, opt-outs, withdrawn consent). They are excluded from retargeting audiences and removed from the ad platforms immediately. Fed by the suppression webhook or added manually below."
      />

      <section className="rounded-lg border border-neutral-200 bg-white p-4">
        <form action={add} className="flex items-end gap-2">
          <label className="flex-1">
            <span className="text-xs font-medium text-neutral-700">Suppress email address</span>
            <input
              name="email"
              type="email"
              required
      ...
      <section className="rounded-lg border border-neutral-200 bg-white p-4">
        <h2 className="text-sm font-semibold text-neutral-900">Suppressed addresses ({rows.length})</h2>
        {rows.length === 0 ? (
          <p className="mt-2 text-xs text-neutral-400">No suppressed addresses yet.</p>
        ) : (
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.email} className="border-b border-neutral-100">
                  ...
                  <td className="text-neutral-500">{new Date(r.createdAt).toLocaleDateString("en-GB")}</td>
