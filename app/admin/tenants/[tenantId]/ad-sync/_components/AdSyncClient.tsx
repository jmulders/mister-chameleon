...
export function AdSyncClient({
  ...
  const toggleEnabled = (val: boolean) => {
    ...
    startTransition(async () => {
      ...
      flash(r.ok ? (val ? "Sync enabled." : "Sync disabled.") : `Error: ${r.error}`);
    });
  };

  const saveSegment = () => startTransition(async () => {
    const r = await saveAdSyncSegmentAction(tenantId, segment);
    flash(r.ok ? "Segment saved." : `Error: ${r.error}`);
  });
  ...
  const savePlatform = (platform: AdPlatform, config: GoogleAdsConfig | MetaConfig | LinkedInConfig) =>
    startTransition(async () => {
      ...
      flash(r.ok ? `${platform} saved.` : `Error: ${r.error}`);
    });
  ...
  const syncNow = () => startTransition(async () => {
    ...
    if (r.ok) { setSyncResults(r.results); flash(`Sync complete: ${r.membersTotal} leads processed.`); }
    else flash(`Error: ${r.error}`);
  });

  const saveConv = () => startTransition(async () => {
    const r = await saveConversionConfigAction(tenantId, conv);
    flash(r.ok ? "Conversion feedback saved." : `Error: ${r.error}`);
  });
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <label className="flex items-center gap-2 text-sm font-medium text-neutral-900">
              ...
              Daily audience sync enabled
            </label>
            <p className="mt-1 text-xs text-neutral-500">Runs nightly (03:30). Pushes the segment to every connected platform.</p>
          </div>
          ...
          <div className="mt-3 space-y-1 text-xs">
            {syncResults.map((r) => (
              <div key={r.platform} className={r.status === "ok" ? "text-green-700" : r.status === "skipped" ? "text-neutral-400" : "text-red-700"}>
                <strong className="capitalize">{r.platform}</strong>: {r.status}, +{r.membersSent} added, −{r.membersRemoved ?? 0} removed (segment: {r.membersTotal}){r.error ? ` (${r.error})` : ""}
              </div>
            ))}
          </div>
        ) : null}
      ...
      {/* Segment definition */}
      <section className="rounded-lg border border-neutral-200 bg-white p-4">
        <h2 className="text-sm font-semibold text-neutral-900">Segment: which leads</h2>
        <p className="mt-1 text-xs text-neutral-500">Only leads with an email address (from your lead base) are included.</p>
        ...
        <label className="mt-3 flex items-center gap-2 text-xs text-neutral-700">
          ...
          Only leads with consent or first-party (recommended)
        </label>
      ...
      <PlatformCard title="Google: Customer Match (Data Manager API)"
        ...
        <Field label="Customer id (target account)" value={google.customerId ?? ""} onChange={(v) => setGoogle({ ...google, customerId: v })} hint="Digits, no dashes" />
        <Field label="Login customer id (MCC)" value={google.loginCustomerId ?? ""} onChange={(v) => setGoogle({ ...google, loginCustomerId: v })} hint="Optional, only for a manager account" />
        <Field label="User list id" value={google.userListId ?? ""} onChange={(v) => setGoogle({ ...google, userListId: v })} hint="The Customer Match list" />
        <Field label="OAuth client id" value={google.clientId ?? ""} onChange={(v) => setGoogle({ ...google, clientId: v })} />
        <Field label="OAuth client secret" type="password" value={isSet(google.clientSecret) ? "" : (google.clientSecret ?? "")} placeholder={isSet(google.clientSecret) ? "•••• saved" : ""} onChange={(v) => setGoogle({ ...google, clientSecret: v })} />
        <Field label="Refresh token" type="password" value={isSet(google.refreshToken) ? "" : (google.refreshToken ?? "")} placeholder={isSet(google.refreshToken) ? "•••• saved" : ""} onChange={(v) => setGoogle({ ...google, refreshToken: v })} hint="Scope: .../auth/datamanager" />
      </PlatformCard>
      ...
      <PlatformCard title="Meta: Custom Audience"
        ...
        <Field label="Access token (system user)" type="password" value={isSet(meta.accessToken) ? "" : (meta.accessToken ?? "")} placeholder={isSet(meta.accessToken) ? "•••• saved" : ""} onChange={(v) => setMeta({ ...meta, accessToken: v })} />
        <Field label="Ad account id" value={meta.adAccountId ?? ""} onChange={(v) => setMeta({ ...meta, adAccountId: v })} hint="act_… or just the digits" />
      ...
      <PlatformCard title="LinkedIn: Matched Audience (DMP Segment)"
        ...
        <Field label="Access token" type="password" value={isSet(linkedin.accessToken) ? "" : (linkedin.accessToken ?? "")} placeholder={isSet(linkedin.accessToken) ? "•••• saved" : ""} onChange={(v) => setLinkedin({ ...linkedin, accessToken: v })} />
        <Field label="Ad account id" value={linkedin.adAccountId ?? ""} onChange={(v) => setLinkedin({ ...linkedin, adAccountId: v })} hint="Digits" />
      ...
      <section className="rounded-lg border border-neutral-200 bg-white p-4">
        ...
        <p className="mt-1 text-xs text-neutral-500">
          On a form conversion, sends a server-side event back (hashed email) so bidding optimises on real leads. Reuses the credentials above; fill in the conversion target per platform.
        </p>
        <div className="mt-3 grid grid-cols-2 gap-3">
          <Field label="Event name" value={conv.eventName ?? ""} onChange={(v) => setConv({ ...conv, eventName: v })} hint="E.g. Lead" />
          <Field label="Default value" type="number" value={conv.defaultValue?.toString() ?? ""} onChange={(v) => setConv({ ...conv, defaultValue: v ? Number(v) : undefined })} hint="Optional, per conversion" />
          <Field label="Currency" value={conv.currency ?? ""} onChange={(v) => setConv({ ...conv, currency: v })} hint="E.g. EUR" />
          <Field label="Google conversion action id" value={conv.google?.conversionActionId ?? ""} onChange={(v) => setConv({ ...conv, google: { conversionActionId: v } })} hint="UPLOAD_CLICKS action" />
      ...
      <section className="rounded-lg border border-neutral-200 bg-white p-4">
        ...
        {runs.length === 0 ? (
          <p className="mt-2 text-xs text-neutral-400">No syncs run yet.</p>
        ) : (
          <>
          <table className="mt-2 w-full text-xs">
            <thead>
              <tr className="text-left text-neutral-400">
                <th className="py-1">Time</th><th>Platform</th><th>Status</th><th>Added</th><th>Removed</th><th>Segment</th><th>Trigger</th>
              </tr>
            </thead>
            <tbody>
              {runsPager.pageItems.map((r) => (
                <tr key={r.id} className="border-t border-neutral-100">
                  <td className="py-1">{new Date(r.createdAt).toLocaleString("en-GB")}</td>
    ...
    <section className="rounded-lg border border-neutral-200 bg-white p-4">
      ...
      <div className="mt-3 flex gap-2">
        ...
        <button onClick={onClear} disabled={pending}
          className="rounded-md border border-red-200 bg-white px-3 py-1.5 text-xs font-medium text-red-700 hover:bg-red-50 disabled:opacity-50">Disconnect</button>
      </div>
    </section>
  );
}
