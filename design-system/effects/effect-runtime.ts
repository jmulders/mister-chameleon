...
/* Scroll-linked (default-off, feature-detected). The runtime writes inline
   opacity (scroll-fade) or --mc-fx-scroll-scale (scroll-scale) from the scroll
   position. Off under reduced-motion (runtime skips + the reduce block below). */
.mc-fx-scroll-fade {
  will-change: opacity;
}
.mc-fx-scroll-scale {
  transform: scale(var(--mc-fx-scroll-scale, 1));
