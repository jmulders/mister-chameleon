 ...
 *
 *   The `Database` type is GENERATED — it lives in ./database.types.ts and is
 *   re-exported at the bottom of this file. Regenerate it after every migration:
 *
 *     npx --yes supabase gen types typescript --linked > data/database.types.ts
 *
 *   The Row/Insert types below stay hand-written: they are the shapes the app
 *   passes around, and they carry documentation the generator cannot. Keep them
 *   honest against the generated file — where the two disagree, the generated
 *   one is right, because it was read from the database.
 *
...
export interface EventRow {
   ...
   *
   * `Json`, not `Record<string, unknown>`: the column is jsonb, which also holds
   * scalars and arrays. Everything the app writes is an object (see EventInsert),
   * but a read cannot assume that — narrow at the read site. The old type made a
...
export interface TenantDomainRow {
  /**
   * bigint primary key (nextval) — NOT a UUID.
   *
   * Checked against the live database. This said `string` and the doc comments
   * on removeDomain/setPrimaryDomain said "UUID"; the column is
   * `bigint generated by nextval('tenant_domains_id_seq')`. Nobody noticed
   * because the hand-written Database type was structurally invalid, so
   * supabase-js resolved every table to `never` and compared nothing to nothing.
   */
  id:                  number;
  /** Owning tenant slug, e.g. "workengine". */
  tenant_id:           string;
 ...
 */
/**
 * The database schema, generated from the live database.
 *
 * Hand-written until 17 July 2026, when it turned out that supabase-js had never
 * accepted it: @supabase/postgrest-js requires `Relationships` on every table,
 * and it was on none. So the type failed its GenericSchema constraint and every
 * table resolved to `never` — which is what the 24 `(db as any)` casts in this
 * codebase were working around.
 *
 * See ./database.types.ts for how to regenerate.
 */
